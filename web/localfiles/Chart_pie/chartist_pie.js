...
voltmxmp.charts.pie = function(){

};

voltmxmp.charts.pie.prototype.createClass = function(name, rules) {
...
voltmxmp.charts.pie.prototype.createPieChart = function(id,labels,series,options,opacity){
...
voltmxmp.charts.pie.prototype.UI_createpieCharts = function(lbls,srs,legends)
{
...
voltmxmp.charts.pie.prototype.Updatecss = function(colors)
{
...
voltmxmp.charts.pie.prototype.setProperties = function(properties){
...
voltmxmp.charts.pie.prototype.Generate_PieChart = function(title,labels,data,colors,properties){
...
onbodyload = function(){
  //workaround for iris issue: replace kony with voltmx
  if(voltmx) {
    var href = window.location.href.substring(0,4);  
    if(href === 'file'){
      var state = [];
      state.push("ready");
      voltmx.evaluateJavaScriptInNativeContext("chart_pie_defined_global", JSON.stringify(state));
    }else{
      voltmx.evaluateJavaScriptInNativeContext("chart_pie_defined_global('ready')");
    } 
  }
}.bind(this);
